        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between relative">
          ...
          <div className="flex items-center gap-10">
            ...
            <div className="hidden md:flex items-center gap-6.5">
              <Link
                href="/"
                className="text-sm font-semibold text-stone-700 hover:text-blue-700 transition-colors"
              >
                Home
              </Link>
              ...
              >
                <button
                  ...
                  className="text-sm font-semibold text-stone-700 hover:text-blue-700 transition-colors flex items-center gap-1 cursor-pointer focus:outline-none"
                >
                  Services
                  <ChevronDown className={`h-4.5 w-4.5 transition-transform duration-300 ${activeDropdown === "services" ? "rotate-180 text-blue-700" : ""}`} />
                </button>
              </div>
              ...
              >
                ...
                >
                  Solutions
                  <ChevronDown className={`h-4.5 w-4.5 transition-transform duration-300 ${activeDropdown === "solutions" ? "rotate-180 text-blue-700" : ""}`} />
                </button>
              </div>
                    ...
                    <div className="flex flex-col gap-3">
                      {column.items.map((item, i) => (
                        <Link
                          ...
                          href={item.href}
                          onClick={() => setActiveDropdown(null)}
                          className="text-sm font-medium text-stone-600 hover:text-blue-700 transition-colors hover:translate-x-1 duration-200 transform inline-block"
                        >
                          {item.name}
                        </Link>
                      ))}
        ...
        <div className="fixed top-0 left-0 w-full bg-white z-50 md:hidden shadow-2xl border-b border-stone-100 animate-slide-down max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex flex-col gap-5 py-6 px-6 text-left">
            ...
            <div className="border-b border-stone-50 pb-2">
              ...
              >
                <span>Services</span>
                <ChevronDown className={`h-5 w-5 transition-transform duration-300 ${mobileActiveTab === "services" ? "rotate-180 text-blue-700" : ""}`} />
              </button>
                ...
                <div className="pl-4 pt-3 pb-2 flex flex-col gap-4 bg-stone-50/50 rounded-md p-3 my-1">
                  {servicesMenu.map((group, idx) => (
                    <div key={idx} className="flex flex-col gap-2">
                      <span className="text-xs font-bold text-blue-700 uppercase tracking-wider">{group.title}</span>
                      {group.items.map((item, i) => (
                        <Link
                          key={i}
                          href={item.href}
                          onClick={() => setIsMobileOpen(false)}
                          className="text-sm font-semibold text-stone-700 hover:text-blue-700 py-1"
                        >
                          {item.name}
                        </Link>
                      ))}
            ...
            <div className="border-b border-stone-50 pb-2">
              ...
              >
                <span>Solutions</span>
                <ChevronDown className={`h-5 w-5 transition-transform duration-300 ${mobileActiveTab === "solutions" ? "rotate-180 text-blue-700" : ""}`} />
              </button>
