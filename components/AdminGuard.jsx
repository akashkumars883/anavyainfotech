...
export default function AdminGuard({ children }) {
  ...
  useEffect(() => {
    ...
    const savedAuth = localStorage.getItem("anavyainfotech_admin_auth");
    if (savedAuth === "true") {
  ...
  const handleLogin = (e) => {
    ...
    if (passcode.trim() === "admin123" || passcode.trim() === "anavyainfotech2026") {
      localStorage.setItem("anavyainfotech_admin_auth", "true");
      setIsAuthenticated(true);
      setError(false);
    } else {
      setError(true);
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center px-6 py-12 text-left selection:bg-blue-600/20">
        <div className="max-w-md w-full bg-white border border-stone-200 rounded-md p-8 space-y-8">
          {/* Logo & Header */}
          <div className="space-y-3 text-center">
            <div className="h-12 w-12 rounded-md bg-blue-700/10 border border-blue-700/20 flex items-center justify-center mx-auto text-blue-700">
          ...
          <div className="text-[11px] text-stone-400 text-center font-light border-t border-stone-100 pt-4">
            Protected Admin Area • Anavya Infotech
          </div>
        </div>
      </div>
    );
