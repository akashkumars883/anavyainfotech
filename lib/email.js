...
const REPLY_TO_EMAIL = process.env.REPLY_TO_EMAIL || smtpUser;


const ADMIN_NOTIFICATION_EMAIL = process.env.ADMIN_EMAIL || "ak706908@gmail.com";
...
export async function sendLeadNotificationEmails({ name, email, phone, service, message, source = "Website Form" }) {
  ...
  const customerContact = email && email.includes("@") ? email.trim() : null;
  const leadPhone = phone || (email && !email.includes("@") ? email.trim() : "N/A");
  ...
  if (ADMIN_NOTIFICATION_EMAIL) {
    try {
      ...
      console.error("Admin Email Send Failed:", e.message);
    }
  }
