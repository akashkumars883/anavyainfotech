...
// Sender email (Resend default test sender or custom verified domain)
const FROM_EMAIL = process.env.RESEND_FROM_EMAIL || "Anavya Infotech <onboarding@resend.dev>";
const ADMIN_NOTIFICATION_EMAIL = process.env.ADMIN_EMAIL || "info@anavyainfotech.com";

/**
 * Sends dual email notifications when a lead/query is submitted.
...
export async function sendLeadNotificationEmails({ name, email, phone, service, message, source = "Website Form" }) {
  ...
  try {
    ...
    if (ADMIN_NOTIFICATION_EMAIL) {
      ...
      results.adminEmail = !!adminEmailResult.data?.id;
    }
    ...
    if (customerContact) {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
            ...
            <div style="padding: 28px; color: #334155; font-size: 14px; line-height: 1.6;">
              ...
              <p style="margin-bottom: 0;">If you need urgent assistance, feel free to reply directly to this email or call us at <strong>+91 98765 43210</strong>.</p>
            </div>
      ...
      results.customerEmail = !!customerEmailResult.data?.id;
    }
