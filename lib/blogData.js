...
function cleanImageUrl(url, title = "", category = "") {
  if (url && typeof url === "string" && url.trim().length > 0) {
    const trimmed = url.trim();
    // Ignore giant inline base64 image URLs (over 50KB) that bloat payloads and exceed Next.js cache limit
    if (trimmed.startsWith("data:") && trimmed.length > 50000) {
...
function cleanContentHtml(html = "") {
  ...
  // Strip or replace inline base64 image src strings over 50KB embedded in content HTML
  return html.replace(/src=["']data:image\/[^;]+;base64,[^"']{50000,}["']/g, 'src="/technology-illustration.jpg"');
}
