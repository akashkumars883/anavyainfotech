...
function cleanImageUrl(url, title = "", category = "") {
  if (url && typeof url === "string" && url.trim().length > 0) {
    const trimmed = url.trim();
    // If image URL is a huge base64 string (>200KB), replace with fallback image to avoid oversized ISR page errors
...
function cleanContentHtml(html = "") {
  ...
  // Strip or replace inline base64 images larger than 300KB in body content
...
async function fetchBlogPostsFromDb() {
  ...
  const mergedPosts = [...dbFormattedPosts, ...missingStaticPosts].map((post, idx) => ({ ...post, featured: idx === 0 }));
  
  // Only cache in Redis if we fetched fresh posts from Supabase DB
  if (dbFormattedPosts.length > 0) {
    await setCache(cacheKey, mergedPosts, 300); // 5 minutes TTL
  }
...
export const getBlogPosts = cache(
  unstable_cache(
    ...
    {
      revalidate: 60, // 60 seconds background revalidation
...
async function fetchSinglePostFromDb(slug) {
  ...
  try {
    ...
    if (!error && dbPost) {
      const formatted = formatDbPost(dbPost, 0);
      await setCache(cacheKey, formatted, 300); // 5 minutes TTL
      return formatted;
    }
  } catch (err) {
  ...
  // Fallback to static blog posts array if DB post is not found (Do NOT cache fallback in Redis)
