...
function formatDbPost(dbPost, index) {
  ...
  const isoDate = dbPost.created_at ? new Date(dbPost.created_at).toISOString() : new Date().toISOString();
  ...
  return {
    ...
    meta_description: dbPost.meta_description || dbPost.excerpt || dbPost.title,
    meta_title: dbPost.meta_title || "",
  };
}
...
async function fetchBlogPostsFromDb() {
  ...
  try {
    const res = await tursoClient.execute(
      "SELECT id, slug, title, category, author, excerpt, image_url, tags, is_published, created_at, views_count, meta_title FROM blogs WHERE is_published = 1 ORDER BY created_at DESC"
    );
