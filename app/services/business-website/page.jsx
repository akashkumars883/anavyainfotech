...
export const metadata = {
  ...
  description:
    "Anavya Infotech is a top website development company in India & USA and premier website design company offering custom Next.js web design services, local SEO setups, and global enterprise platforms.",
  keywords:
...
export default function BusinessWebsitePage() {
  const scopes = [
    ...
    },
    {
      ...
      icon: <Compass className="h-6 w-6 text-blue-700" />,
      ...
      <section className="py-10 bg-stone-50 border-b border-stone-100 px-6">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-white border border-stone-200 text-[11px] font-bold uppercase tracking-wider text-stone-600">
            <Code className="h-3.5 w-3.5 text-blue-700" /> Services / Website Development Company
          </div>
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-normal tracking-tight text-stone-900 leading-[1.1] max-w-5xl">
            ...
            <span className="text-blue-700">India & USA – Custom Web Design</span>
          </h1>
          <p className="text-base sm:text-lg md:text-xl text-stone-600 font-light max-w-3xl leading-relaxed">
            Anavya Infotech is a premier <strong>website design company in India</strong> and <strong>website development company in USA</strong>. We deliver custom <strong>web development services</strong> tailored for high performance, conversion rate optimization, and search engine authority.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto space-y-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {scopes.map((scope, index) => (
              <article
                key={index}
                className="bg-white border border-stone-100 rounded-md p-8 flex flex-col justify-between hover:border-stone-300 hover:shadow-xl transition-all duration-300"
              >
                <div className="space-y-6">
                  <div className="h-12 w-12 rounded-md bg-stone-50 border border-stone-100 flex items-center justify-center">
                    {scope.icon}
                  </div>

                  <div className="space-y-2">
                    <h3 className="text-xl font-bold tracking-tight text-stone-900">{scope.title}</h3>
                    <div className="text-xs font-semibold text-blue-700 uppercase tracking-wider">{scope.tagline}</div>
                  </div>
                  ...
                  <ul className="space-y-2.5 pt-2 border-t border-stone-50">
                    {scope.benefits.map((benefit, i) => (
                      <li key={i} className="flex items-start gap-2.5 text-xs text-stone-700">
                        <Zap className="h-3.5 w-3.5 text-blue-700 shrink-0 mt-0.5" />
                        <span>{benefit}</span>
                      </li>
                    ))}
                  </ul>
