...
export async function generateMetadata({ params }) {
  ...
  if (!project) {
    return { title: "Case Study Not Found – Anavya Infotech" };
  }
...
export default async function CaseStudyDetailPage({ params }) {
  ...
  return (
    <main className="min-h-screen bg-white pt-32 sm:pt-32 md:pt-28 pb-20 text-left selection:bg-blue-600/20 selection:text-blue-950">
      <script
      ...
      <div className="max-w-5xl mx-auto px-6 space-y-12">
        ...
        <header className="space-y-6 border-b border-stone-200 pb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 p-6 rounded-md bg-stone-50 border border-stone-200">
            <div>
            ...
            {project.websiteUrl && (
              <div>
                ...
                <a
                  ...
                  className="text-xs font-bold text-blue-700 hover:underline inline-flex items-center gap-1 mt-1"
                >
        ...
        {project.image && (
          <div className="w-full h-64 sm:h-96 rounded-md overflow-hidden border border-stone-200 shadow-sm relative group">
            <img
              src={project.image}
              alt={project.title}
            ...
            {project.websiteUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="absolute bottom-4 right-4 inline-flex items-center gap-2 bg-black/80 backdrop-blur-md text-white text-xs font-bold px-5 py-2.5 rounded-md hover:bg-black transition-all shadow-lg border border-white/20"
              >
                Visit Live Website <ArrowUpRight className="h-3.5 w-3.5 text-blue-600" />
              </a>
            )}
          </div>
        )}
        ...
        {/* Key Metrics Banner */}
        <section className="p-8 rounded-md bg-stone-900 text-white space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-2">
            {project.metrics.map((metric, i) => (
              <div key={i} className="p-4 rounded-md bg-white/10 border border-white/10 text-center">
        ...
        <section className="space-y-10 text-stone-700 text-base leading-relaxed font-light">
          ...
          <div className="space-y-4">
            ...
            <div className="p-6 rounded-md bg-stone-50 border border-stone-200 text-stone-600 text-sm sm:text-base leading-relaxed">
          ...
          <div className="space-y-4">
            ...
            <div className="p-6 rounded-md bg-white border border-stone-200 text-stone-600 text-sm sm:text-base leading-relaxed">
          ...
          <div className="space-y-4">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {project.keyFeatures.map((feat, i) => (
                <li key={i} className="p-4 rounded-md bg-stone-50 border border-stone-200 text-xs sm:text-sm font-semibold text-stone-800 flex items-start gap-3">
          ...
          <div className="space-y-4 pt-4 border-t border-stone-200">
            ...
            <div className="flex flex-wrap gap-2">
              {project.tags.map((tag, i) => (
                <span key={i} className="px-4 py-2 rounded-md text-xs font-semibold bg-stone-100 border border-stone-200 text-stone-800">
                  {tag}
                </span>
              ))}
            </div>
        ...
        <footer className="pt-10 border-t border-stone-200 space-y-12">
          <div className="p-8 rounded-md bg-black text-white flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 px-6 py-3.5 rounded-md text-xs font-bold uppercase tracking-wider bg-white text-black hover:bg-zinc-200 transition-colors shrink-0 shadow-lg"
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {relatedProjects.map((rel, i) => (
                  <Link
                    ...
                    className="p-6 rounded-md bg-stone-50 border border-stone-200 hover:border-stone-400 transition-all space-y-3 block group"
                  >
                    <span className="text-[10px] font-bold text-blue-700 uppercase tracking-widest">{rel.category}</span>
                    <h4 className="text-lg font-bold text-stone-900 group-hover:text-blue-700 transition-colors">{rel.title}</h4>
